...
StockSim <- function(Y,N,T,sim,nruns,d="stock"){
  ...
  for(i in c(1:num_runs)){
    ...
    est_model_MCPanel <- mcnnm(Y_obs, treat_mat, W= p.weights, to_estimate_u = 1, to_estimate_v = 1, lambda_L = c(0.05))[[1]] # no CV to save computational time    
