...
# Load data
Y.train <- read.csv('data/sine_train_real.csv',header=F) # real sine waves

Y <- t(Y.train) # N X T
print(dim(Y))

# fixed dimensions
results <- foreach(T = c(140,700,1400,dim(Y)[2]), .combine='rbind') %do% {
  SineSim(Y,N=dim(Y)[1],T)
}
saveRDS(results, "results/sine-placebo-results.rds")