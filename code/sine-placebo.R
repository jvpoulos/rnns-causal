...
SineSim <- function(Y,N,T,sim){
  ...
  t0 <- ceiling(T*0.75) # time of initial treatment
  N_t <- ceiling(N/2)
  num_runs <- 60
  ...
  for(i in c(1:num_runs)){
    ...
    p.mod <-   glmnet(x=Y_obs, y=(1-treat_mat)[,t0], lambda=c(0.01), thresh = 1e-05, family="binomial")
    ...
    print("ED Started")
    source("code/ed.R")
    est_model_ED <- ed(Y=Y_obs, p.weights, trends, treat_indices, d, t0, T)
