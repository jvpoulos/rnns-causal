...
CapacitySim <- function(outcomes,covars.x,d,sim,imp,if.save=TRUE){
  ...
  for(j in T0){
    ...
    p.mod <- cv.glmnet(x=cbind(capacity.covars,Y_obs[,1:(t0.placebo-1)]), y=(1-treat_mat), family="mgaussian")
    W <- predict(p.mod, cbind(capacity.covars,Y_obs[,1:(t0.placebo-1)]), s = "lambda.min")[,,1]
