...
basque.ape.df <- data.frame("Year"=1990:1997,
...
basque.ape.plot <- ggplot(data=basque.ape.df, aes(x=Year)) +
  ...
  scale_linetype_manual("Model",values=c("BSTS"=3,"Encoder-decoder"=4,"Synthetic control"=5),
                        labels=c("BSTS", "Encoder-decoder","Synthetic control")) +
  scale_colour_manual(name="Model", values = c("BSTS" = wes_palette("Darjeeling")[4], "Encoder-decoder" = wes_palette("Darjeeling")[2], "Synthetic control" = wes_palette("Darjeeling")[5]),
                      labels=c("BSTS", "Encoder-decoder", "Synthetic control")) +
