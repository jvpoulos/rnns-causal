...
# Priors
prior.test <- SpikeSlabPrior(x=model.matrix(gdp.10 ~ ., data=cbind(germany.y.train[-1], germany.x.train[-1])), 
                             y=germany.y.train$gdp.10, 
                             prior.information.weight = 0.01)

## Construct state components

ss.test <- list()
ss.test <- AddSemilocalLinearTrend(ss.test, germany.y.train$gdp.10) # Semilocal Linear Trend
ss.test <- AddSeasonal(ss.test,germany.y.train$gdp.10,
                  nseasons = 52) # monthly seasonal component 

bsts.reg.test <- bsts(gdp.10 ~ .,
                      data = cbind(germany.y.train[-1], germany.x.train[-1]),
                      state.specification = ss.test,
                      prior=prior.test,
                      niter = 1000,
                      ping = 0, seed = 2016)

saveRDS(bsts.reg.test, paste0(data.directory, "bsts-reg-test-germany.rds"))
...
d2 <- data.frame(
  ...
  c(as.numeric(-colMeans(bsts.reg.test$one.step.prediction.errors[-(1:burn.test),])+germany.y.train$gdp.10),  
...
bsts.plot <- ggplot(data=d3, aes(x=Date)) +
  ...
  ggtitle(paste0("Germany (placebo): BSTS (training MSPE = ", round(bsts.MSPE,3), ")")) +
  theme.blank 

ggsave(paste0(results.directory,"plots/bsts-plot-germany.png"), bsts.plot, width=11, height=8.5)

# Post-period MSPE

germany.bsts.MSPE <- filter(d3, Date %in% c(1990:2003)) %>% mutate(MSPE=mean((Actual-Fitted )**2))
germany.bsts.MSPE$MSPE[1]

# Absolute percentage estimation error
germany.bsts.APE <- filter(d3, Date %in% c(1990:2003)) %>% mutate(APE=abs(Fitted-Actual)/abs(Actual))
mean(germany.bsts.APE$APE)