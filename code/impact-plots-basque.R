...
setwd(paste0(results.directory, "basque")) # prediction files loc

test.files <- list.files(pattern = "*test.csv")

basque.preds.test <- lapply(test.files,function(x){
  m <- read.csv(x, header=FALSE)[8,] # get last sample
  return(as.matrix(m))})
...
ed.plot <- ggplot(data=basque.bind, aes(x=year)) +
  ...
  ggtitle(paste0("Basque Country: Encoder-decoder (validation MSPE = 0.22)")) +
  theme.blank 

ggsave(paste0(results.directory,"plots/impact-basque.png"), ed.plot, width=11, height=8.5)
