...
SalesSim <- function(Y,N,T,sim,nruns){
...
# Fixed dimensions: NxT=361,000
N.seq <- seq(190,1900, by=190)
T.seq <- round(190*1900/N.seq)

for(sim in c(0,1)){
  for(n in 1:length(N.seq)){
    SalesSim(Y,N=N.seq[n],T=T.seq[n],sim=sim,nruns=10)
  }
}

for(sim in c(0,1)){
  SalesSim(Y,N=500,T=dim(Y)[2],sim=sim,nruns=100) 
}