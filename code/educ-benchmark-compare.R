...
CapacityCompare <- function(imp=c("none","knn","locf","linear","ma","mean","mice","random","rf"), run.CI=TRUE, if.save=TRUE){
  ...
  if(if.save){
    df1 <-
      data.frame(
        "y" =  c(mean(DID_ATT),mean(MCPanel_ATT),mean(ADH_ATT),mean(ENT_ATT),mean(VAR_ATT)),
        "CI.lower" = c(mean(DID_CI_treated[,1], na.rm=TRUE),mean(MCPanel_CI_treated[,1], na.rm=TRUE),mean(ADH_CI_treated[,1], na.rm=TRUE),mean(ENT_CI_treated[,1], na.rm=TRUE),mean(VAR_CI_treated[,1], na.rm=TRUE)),
        "CI.upper" = c(mean(DID_CI_treated[,2], na.rm=TRUE),mean(MCPanel_CI_treated[,2], na.rm=TRUE),mean(ADH_CI_treated[,2], na.rm=TRUE),mean(ENT_CI_treated[,2], na.rm=TRUE),mean(VAR_CI_treated[,2], na.rm=TRUE)),
        "Method" = c("DID", 
                     "MC-NNM", 
                     "SCM",
                     "SCM-L1",
