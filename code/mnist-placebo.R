...
MNISTSim <- function(Y,N,T,sim){
  ...
  for(i in c(1:num_runs)){
    ...
    est_model_ENT <- t(en_mp_rows(t(Y_obs), t(treat_mat), num_alpha = 1, num_lam = 5, num_folds = nrow(t(Y_obs)))) # LOOCV avoid constant y
...
Y <- read.csv('data/mnist_train.csv',header=F)
Y <- Y[,-1] # remove labels
Y <- as.matrix(Y)
