...
LinearSim <- function(Y,N,T){
  ...
  for(i in c(1:num_runs)){
    ...
    est_model_EN <- en_mp_rows(Y_obs, treat_mat, num_lam = 5, num_alpha = 1, num_folds = 5)
...
Y.train <-  read.csv('data/linear_train_real.csv',header=F)
Y.val <- read.csv('data/linear_val_real.csv',header=F) 
Y.test <- read.csv('data/linear_test_real.csv',header=F)
